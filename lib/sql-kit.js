var sqlOps = {
    assureAllTables: async (pth) => {
        ...
        for (var key in sqlOps.tables) {
            var result = await query(sql, ["jurismDB", "BASE_TABLE", key]);
            if (!result.length) {
                console.log(`ALERT: Creating ${key} table`);
                await query(sqlOps.tables[key].createStatement());
            }
        }
    },
    tables: {
        ...
        styles: {
            ...
            createStatement: function() {
                ...
                var fields = lst.join(",");
                var ret =  `CREATE TABLE styles (${fields})`;
                return ret;
            },
            insertParams: function(info) {
                var ret = [];
        ...
        bugs: {
            ...
            createStatement: function() {
                ...
                var ret =  `CREATE TABLE bugs (${fields})`;
                return ret;
            }
        }
    }
