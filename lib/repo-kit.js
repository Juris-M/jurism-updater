...
function assureCommitHashTable(val) {
    ...
    return conn.then((conn) => conn.query(sql, ["jurism", "commit_hash"]))
        .then((results) => {
...
function readCommitHash(repoName) {
    ...
    return conn.then((conn) => conn.query(sql, [repoName]))
        .then((results) => {
            if (results[0].length > 0) {
...
function writeCommitHash(repoName, commitHash) {
    return deleteCommitHash(repoName)
        .then(() => {
            ...
            return conn.then((conn) => conn.query(sql, [repoName, commitHash]))
        })
}
