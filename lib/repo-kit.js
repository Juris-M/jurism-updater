...
function replaceFile(repoName, fn) {
    ...
    return removeFile(repoName, fn)
        .then(() => addFile(repoName, fn))
}

function addFile(repoName, fn) {
    var data = getInfoAndTranslator(fn);
    var info = data.info;
    info.code = data.translator;
    return getFileDate(repoName, fn)
        .then((d) => {
            info.lastUpdated = d;
...
function checkTable(repoName, goal) {
    // Check the number of entries currently in the table against goal,
    ...
    return query(sql)
        .then((result) => {
            if (result[0][0].count == goal) {
                return reportRepoTime(true);
            } else {
                return {
                    done: false
                }
...
function populateTable(repoInfo) {
    ...
    return pullChanges(repoInfo)
        ...
        .then((filenames) => {
            // Set an orphan promise to generate the DB,
            // and return the number of entries expected
            // to appear in the table when it is complete
            // as "goal."
            Promise.each(filenames, function(fn){
                replaceFile(repoInfo.name, fn)
            });
            return {
                goal: filenames.length
            }
        });
 }

function reportRepoTime(done) {
