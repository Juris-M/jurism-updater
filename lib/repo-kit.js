...
var pool = require(pth.fp.connection).pool;
...
async function addFile(repoInfo, fn, db) {
    debug("addFile()");
...
async function recreateTables(targets) {
    ...
    await pool.transaction(async db => {
        for (var repoName of repoNames) {
            ...
            var results = await query(sql, params, db);
...
async function dropTable (repoInfo, db) {
    debug("dropTable()");
    var sql = `DROP TABLE ${repoInfo.category}`;
    return await query(sql, null, db);
}

async function createTable(repoInfo, db) {
    debug("createTable()");
    var sql = sql_kit[repoInfo.category].createStatement(repoInfo);
    return await query(sql, null, db);
}

async function populateTable(repoInfo, db) {
    debug("populateTable()");
    await pullChanges(repoInfo);
    // Ooooh, needs help!
    var filenames = [];
    ...
    console.log(`WHAT THE FUCK? filenames.length=${filenames.length}`);
    for (var fn of filenames) {
        console.log(`  FILENAME: ${fn} AND FUCKING styleType=${repoInfo.dir}`);
        await addFile(repoInfo, fn, db);
    }
